...
- Celery Beat (定时任务 - 清理过期文件、更新寿星称号)
...
3. **IsAdmin** - 所有操作需管理员
   - 用于：人事管理 (`/api/v1/personnel/`)、外请教师、称号等

4. **IsAdminOrSelfReadOnly** - 读需登录，写需管理员
   - 用于：队员信息 (`/api/v1/personnel/members/`)

5. **AllowAny** - 无需认证（仅特殊端点）
...
- TaCOS 2.1 增加校友状态：管理员手动将成员 `status` 改为 `ALUMNI`，系统创建 `AlumniProfile` 联系窗口。
...
**API调用**:
```bash
curl -X POST http://localhost:8000/api/v1/personnel/titles/update-birthday-titles/ \
...
- 日志：记录在`/tmp/tacos_cron.log`

#### 手动触发
- 前端：称号管理页面的"更新本月寿星"按钮
- API：`POST /api/v1/personnel/titles/update-birthday-titles/`
...
2. **Cron任务不执行**
   - 检查：`service cron status`
   - 验证：`python manage.py crontab show`
   - 日志：`tail -f /tmp/tacos_cron.log`
